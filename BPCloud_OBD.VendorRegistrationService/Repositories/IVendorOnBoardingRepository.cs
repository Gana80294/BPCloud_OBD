...
namespace BPCloud_OBD.VendorRegistrationService.Repositories
{
    public interface IVendorOnBoardingRepository
    {
        ...
        bool attachment_MSME(int Transid, int MSME_ID);
        bool attachment_RP_ID(int Transid, int RP_ID);
        bool attachment_TDS(int Transid, int TDS);
        bool bank_FTP(int TransID, Byte[] AttachmentFile, string AttachmentName);
        bool SendAllAttachmentsToFTP(BPVendorOnBoarding bPVendorOnBoarding);
        bool CreateXMLFromVendorOnBoarding(BPVendorOnBoarding bPVendorOnBoarding);
    }
}
