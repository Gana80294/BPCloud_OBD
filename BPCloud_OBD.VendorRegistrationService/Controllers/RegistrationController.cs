...
namespace BPCloud_OBD.VendorRegistrationService.Controllers
{
      ...
      /*  public List<BPAttachment> GetAttachmentFile(int TransID, string attachmentname)
        ...
        public async Task<IActionResult> ApproveVendor(BPVendorOnBoarding VendorOnBoarding)
        {
            try
            {
                ...
                if (result != null)
                {
                    bool status =_VendorOnBoardingRepository.CreateXMLFromVendorOnBoarding(result);
                    bool status1 = _VendorOnBoardingRepository.SendAllAttachmentsToFTP(result);
        ...
        public IActionResult ApproveVendor_MSMEFTP(int TransID, int MSME_ID, int RP_ID, int TDS)
        {
            try
            {
                bool status2 =_VendorOnBoardingRepository.attachment_MSME(TransID, MSME_ID);
                bool status3 =_VendorOnBoardingRepository.attachment_RP_ID(TransID, RP_ID);
                bool status = _VendorOnBoardingRepository.attachment_TDS(TransID, TDS);
                if (status2)
                {
                    WriteLog.WriteToFile("Registration/ApproveVendor", "Approved Vendor details uploaded to FTP");
                }
        ...
        public IActionResult ApproveVendor_Bank(int TransID)
        {
            ...
            try
            {
                ...
                var doc = _VendorOnBoardingRepository.bank_FTP(TransID, bank1[0].AttachmentFile, bank1[0].AttachmentName);
